...
# FIX: Refactored core game logic out of app.py into logic_utils.py using AI
# agent mode, then imported it here to separate logic from the Streamlit UI.
...
# FIX: This message was hard-coded to "between 1 and 100"; now uses low/high
# so it matches the selected difficulty.
st.info(
...
if submit:
    ...
    if not ok:
        ...
        # FIX: Removed a glitch that cast the secret to a string on even
        # attempts, which made check_guess compare numbers as text and flip
        # the hint direction. Secret now stays an int every turn.
