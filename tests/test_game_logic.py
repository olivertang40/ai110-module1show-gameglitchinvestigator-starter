...
# rewarding the player for a wrong guess. All wrong guesses should deduct points.
...
def test_score_too_high_even_attempt():
    ...
    assert result == -5, f"Expected -5 but got {result} (even-attempt parity bug may have returned)"
...
def test_score_win_decreases_with_attempts():
    # Win score should decrease the longer the player takes, with a floor of 10
    early_win = update_score(0, "Win", 1)   # 100 - 10*(1+1) = 80
    late_win  = update_score(0, "Win", 9)   # 100 - 10*(9+1) = 0 -> clamped to 10
    assert early_win == 80
    assert late_win == 10
